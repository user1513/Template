...
u8 mf_readdir(void)
{
 ...
 /*
 depth = 0只扫描path路径当前目录
 depth = 1扫描path路径下所有文件
 */
...
{
	//如果给出了空指针，它将为函数提供默认值的所有选项
	return f_mkfs((const TCHAR*)path, 0, NULL, au);//格式化,drv:盘符;mode:模式;au:簇大小
} 
//删除文件/目录
//pname:文件/目录路径+名字
//返回值:执行结果
...
/*1. f_mount()注册/取消注册卷的工作区*/
	...
	// while(i--)
	// {
	// 	res = f_lseek(&g_fil,f_size(&g_fil));	/*移至文件末尾以追加数据*/ 
